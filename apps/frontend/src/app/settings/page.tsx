...
export default function SettingsPage() {
  ...
  const validTabs = ["hardware", "nutrients", "facility", "system"];
    ...
    <AppShell>
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex gap-6">
          {/* Desktop Sidebar */}
          <div className="hidden sm:flex sm:flex-col sm:flex-shrink-0 transition-all duration-200"
               style={{ width: sidebarCollapsed ? '3.5rem' : '12rem' }}>
            <button
              onClick={() => setSidebarCollapsed(!sidebarCollapsed)}
              className="w-full px-3 py-2 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg transition flex items-center justify-center"
          ...
          <div className="sm:hidden overflow-x-auto flex space-x-4 pb-2">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              const isActive = activeTab === tab.id;
              return (
                <button
                  key={tab.id}
                  onClick={() => handleTabChange(tab.id)}
                  className={`whitespace-nowrap px-3 py-2 rounded-lg transition flex items-center gap-1.5 ${
                    isActive
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Nutrients (Fertigation Blueprints) Tab */}
            {activeTab === "nutrients" && (
